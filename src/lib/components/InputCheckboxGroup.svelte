<script lang="ts">
  ...
  function findCheckedIndices(items: LabeledValue[], checked: (string | LabeledValue)[] | undefined): number[] {
    if (!checked || checked.length === 0) return [];
    return items.reduce((acc: number[], { value }, index: number) => {
      const isValueChecked = checked.some((checkedItem) => typeof checkedItem === 'string' ? checkedItem === value : 'value' in checkedItem && checkedItem.value === value);
...
<div class={cn('', className)}>
  <Label for="checkbox-group" class="font-regular font-sans text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 dark:text-white">
    {label}
  </Label>
  <ul class="flex {orientation === FormOrientation.Vertical ? 'flex-col' : 'grid grid-cols-4 md:grid-cols-2 lg:grid-cols-4'}">
    {#each data as item, index}
      <li class="flex items-center space-x-1 pb-1 {orientation === FormOrientation.Horizontal ? 'pr-2' : ''}">
        ...
        >
          <Checkbox.Indicator class="inline-flex size-[15px] items-center justify-center text-white">
        ...
        <Label for={`checkbox-${index}`} class="peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
          {item.label}
        </Label>
      </li>
    {/each}
